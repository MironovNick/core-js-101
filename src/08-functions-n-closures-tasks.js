...
function getPolynom() {
  ...
  return function (value) {
    let res = null;
    for (let i = 0; i < args.length; i++) { res += args[i] * Math.pow(value, args.length - 1 - i); }
...
function partialUsingArguments(fn) {
  const args = [];
  if (arguments.length > 1) { for (let i = 1; i < arguments.length; i++) args.push(arguments[i]); }
  return function () {
    if (arguments.length > 0) { for (let i = 0; i < arguments.length; i++) args.push(arguments[i]); }
