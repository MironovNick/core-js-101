...
function isBracketsBalanced(str) {
  ...
  for (let i = 0; i < str.length; i++) {
    if (leftB.indexOf(str[i]) !== -1) { leftArr.push(str[i]); } else {
      if (leftArr.length === 0) { return false; }

      const lB = leftArr.pop();
      if (leftB.indexOf(lB) !== rightB.indexOf(str[i])) { return false; }
    }
  }
  ...
  if (pathes.length > 0) {
    ...
    while (go) {
      ...
      for (let j = 0; j < pathes.length; j++) {
        if (s === '') { s = pathes[j][i]; } else if (s !== pathes[j][i]) {
          go = false;
          break;
        }
      }
...
function getMatrixProduct(m1, m2) {
  const r_m1 = m1.length; const
    c_m1 = m1[0].length;
  const r_m2 = m2.length; const
    c_m2 = m2[0].length;
  const res = [];
  if (c_m1 !== r_m2) return false;

  for (let i = 0; i < r_m1; i++) { res[i] = []; }
